var str=[ {author: "Nelson Mandel", quote: "The greatest glory in living lies not in never falling, but in rising every time we fall."},
...
button.addEventListener("click",random_quote);
function random_quote(){
	let index=Math.floor(Math.random() * str.length);
	let randomQuote= str[index].quote;
	let authorQuote= str[index].author;
	quoteText.textContent=randomQuote;
	authorText.textContent=authorQuote;
}